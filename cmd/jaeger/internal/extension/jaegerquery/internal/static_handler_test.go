...
func TestNotExistingUiConfig(t *testing.T) {
	...
	require.ErrorContains(t, err, "cannot open index.html")
...
func TestRegisterStaticHandlerPanic(t *testing.T) {
	...
	assert.Contains(t, buf.String(), "cannot open index.html")
}
...
func TestHotReloadUIConfig(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Skip hot reloading test on Windows because of file locking")
	}
	dir := t.TempDir()
	...
	run := func(description string, testCase testCase) {
		t.Run(description, func(t *testing.T) {
			...
			if testCase.expectedError != "" {
				if testCase.configFile == "invalid" {
					assert.Contains(t, err.Error(), "cannot read UI config file invalid: open invalid:")
				} else {
