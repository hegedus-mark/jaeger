...
import (
	...
	"github.com/jaegertracing/jaeger/internal/storage/v2/api/metricstore/disabled"
	...
	for v2ID, v1ID := range requestedIDs {
		if _, ok := foundIDs[v2ID]; !ok {
			traceErrors = append(traceErrors, structuredError{
				Msg:     querysvc.ErrTraceNotFound.Error(),
...
func (aH *APIHandler) getTrace(w http.ResponseWriter, r *http.Request) {
	...
	traces, err := v1adapter.V1TracesFromSeq2(getTracesIter)
	if errors.Is(err, querysvc.ErrTraceNotFound) || (err == nil && len(traces) == 0) {
		aH.handleError(w, querysvc.ErrTraceNotFound, http.StatusNotFound)
		return
	}
	if aH.handleError(w, err, http.StatusInternalServerError) {
		return
...
func (aH *APIHandler) archiveTrace(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, querysvc.ErrTraceNotFound) {
